...
fig.figure_.suptitle("Confusion Matrix for HugaDB Dataset")
plt.show()
...
def OptiLIME_loss(kernel_width):
    ...
    return single_lime.score(clustered_data2[x,:], clf.predict_proba)
...
for i in range(0, 10):
        ...
        fig_elime, r_features = exp_elime.as_pyplot_to_figure(type='h', name = i+.8, label='0')
        ...
        fig_elime, r_features = exp_lime.as_pyplot_to_figure(type='h', name = i+.9, label='0')
        lime_exp.append(r_features)
     
################################################
sim = jaccard_distance(elime_exp)
np.savetxt("results/clf_dlime_jdist_ildp.csv", sim, delimiter=",")
